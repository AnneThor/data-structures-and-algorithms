...
const toTitleCase = (arr) => {
  return arr.map((element) => {
    if (!(/^[A-Z]/).test(element)) {
      let updatedElement = element.split('')
      updatedElement[0] = updatedElement[0].toUpperCase()
...
const lukeMass = parseInt(starWarsData.find(char => char.name === 'Luke Skywalker').mass)

let biggerThanLuke = (arr) => {
  let result = []
  arr.forEach(char => {
    if (parseInt(char.mass) > lukeMass) {
      return result.push(char.name)
    }
  })
  return result.join(' - ')
};
