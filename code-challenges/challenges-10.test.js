...
const getNames = (arr) => {
  return arr.map((person) => {
    return person.name = person.name.split('').reverse().join('')
  })
};
...
const count = (target, input) => {
  ...
  input.forEach((innerArray) => {
    fullArr = fullArr.concat(innerArray)
  })
  return fullArr.reduce((acc, val) => {
    if (val === target) {
      return acc+=1
    } else {
      return acc
...
let findShortest = (data) => {
  return data.reduce((acc, character) => {
    if (parseInt(character.height) < parseInt(acc.height)) {
