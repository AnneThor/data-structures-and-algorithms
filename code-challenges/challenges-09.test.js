...
const countNumberOfChildren = (arr) => {
  let children = 0
  return arr.reduce((acc, objVal) => {
    if (!objVal.children) {
      return acc
    } else {
      return acc + objVal.children.length
    }
  }, 0)
};
...
const calculateAverage = (arr) => {
  let totalObj = arr.reduce((acc, val) => {
...
const extractStat = (statName, arr) => {
  let stat = arr.reduce((acc, val) => {
    val.stat.name === statName ?
      acc = val : acc
    return acc
  }, {})
  return stat.hasOwnProperty("stat") ? stat : null
};
...
const extractChildren = (arr) => {
  let regex = /[a]/
  let aChar = arr.filter((char) => {
   return char.name.match(regex)
  })
  return aChar.reduce((acc, value) => {
